 ...
 */
void vmx_enable_intr_window(struct vcpu *v, struct hvm_intack intack);
static void vmx_inject_event(const struct x86_event *event)
{
    ...
    {
        printk("[DEBUG][GOGU] founf valid intr_info: old:0x%016lX new:0x%08X : %d : %d.\n", intr_info, _event.vector, _event.type, _event.error_code);
        HVMTRACE_0D(GOGU);

        if (MASK_EXTR(intr_info, INTR_INFO_INTR_TYPE_MASK) == X86_EVENTTYPE_HW_EXCEPTION ) 
        {
            printk("[DEBUG][GOGU] hvm combine exception.\n");
            _event.vector = hvm_combine_hw_exceptions(
            (uint8_t)intr_info, _event.vector);
            if ( _event.vector == TRAP_double_fault )
                _event.error_code = 0;
        } else if (MASK_EXTR(intr_info, INTR_INFO_INTR_TYPE_MASK) == X86_EVENTTYPE_EXT_INTR)
	{
            printk("[DEBUG][GOGU] enable intr window.\n");
            curr->arch.hvm_vmx.exec_control |= CPU_BASED_VIRTUAL_INTR_PENDING;
	    vmx_update_cpu_exec_control(curr);
	}
    }
...
static void vmx_propagate_intr(unsigned long intr)
{
    ...
    printk("[DEBUG][GOGU] vmx_propagate_intr - %ld. \n", intr);
