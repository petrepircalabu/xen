...
int xc_mem_acquire_resource_mock(xc_interface *xch, uint32_t domain_id,
                                 int param, uint32_t nr, xen_pfn_t *pages)
{
    int rc;
    DECLARE_HYPERCALL_BOUNCE(pages, nr * sizeof(xen_pfn_t), XC_HYPERCALL_BUFFER_BOUNCE_OUT);

    xen_mem_acquire_resource_t res = {
        .domid      = domain_id,
        .type       = XENMEM_resource_mock,
        .id         = param,
        .nr_frames  = nr,
        .flags      = 0,
        .frame      = 0,
    ...
    {
        PERROR("Could not bounce memory for XENMEM_resource_mock");
        return -1;
    }
